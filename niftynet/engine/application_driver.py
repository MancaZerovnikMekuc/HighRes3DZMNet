...
class ApplicationDriver(object):
    ...
    def loop(application,
        ...
        for iter_msg in iteration_messages:
            ...
            # run an iteration
            [bestLoss, trainLoss] = ApplicationDriver.loop_step(application, iter_msg, bestLoss, trainLoss)
    ...
    @staticmethod
    def loop_step(application, iteration_message, bestloss, train_loss):
        """
        ...
        iteration_message.save_iter = False
        numOfGpu = len(iteration_message.current_iter_output['niftynetconsole'])
        if iteration_message.is_training and application.action_param.validation_every_n > 0 and iteration_message.current_iter % application.action_param.validation_every_n == 0:
            loss1 = iteration_message.current_iter_output['niftynetconsole']['loss']
            if(numOfGpu == 2):
                loss2 = iteration_message.current_iter_output['niftynetconsole']['loss_1']
                train_loss = (loss1 + loss2) / 2
            else:
                train_loss = loss1
        if iteration_message.is_validation:
            loss1 = iteration_message.current_iter_output['niftynetconsole']['loss']
            current_loss = (loss1 + train_loss)/2
            if(numOfGpu == 2):
                loss2 = iteration_message.current_iter_output['niftynetconsole']['loss_1']
                current_loss = (((loss1 + loss2) / 2) + train_loss)/2
            if (current_loss < bestloss and (bestloss - current_loss) > 0.0005 and iteration_message.current_iter > 50):
                bestloss = current_loss
                #tf.logging.info('new best ' + bestloss)
                iteration_message.save_iter = True
        if iteration_message.is_training and application.action_param.validation_every_n < 0:
            loss1 = iteration_message.current_iter_output['niftynetconsole']['loss']
            if(numOfGpu == 2):
                loss2 = iteration_message.current_iter_output['niftynetconsole']['loss_1']
                train_loss = (loss1 + loss2) / 2
            ...
            if current_loss < bestloss and (bestloss - current_loss) > 0.0005 and iteration_message.current_iter > 100:
                bestloss = current_loss
                iteration_message.save_iter = True

        # broadcasting event of finishing an iteration
        ITER_FINISHED.send(application, iter_msg=iteration_message)
        return [bestloss, train_loss]
