...
def sensitivity_specificity_loss(prediction,
    ...
    if weight_map is not None:
        ...
        tf.logging.warning('Weighting...')
        ground_truth = tf.multiply(ground_truth, weight_map)
        w = tf.reshape(tf.tile(weight_map, [2]), tf.shape(prediction))
        prediction = tf.multiply(prediction, w)
    ...
    one_cold = 1 - one_hot
    if weight_map is not None:
        w = tf.reshape(tf.tile(weight_map, [2]), tf.shape(one_cold))
        one_cold = tf.multiply(one_cold, w)
...
def dice_class_one(prediction, ground_truth, weight_map=None):
    prediction = tf.cast(prediction, tf.float32)
    if len(ground_truth.shape) == len(prediction.shape):
        ground_truth = ground_truth[..., -1]
    one_hot = labels_to_one_hot(ground_truth, tf.shape(prediction)[-1])
...
def dice_class_two(prediction, ground_truth, weight_map=None):
    prediction = tf.cast(prediction, tf.float32)
    if len(ground_truth.shape) == len(prediction.shape):
        ground_truth = ground_truth[..., -1]
    one_hot = labels_to_one_hot(ground_truth, tf.shape(prediction)[-1])
