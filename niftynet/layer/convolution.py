...
from niftynet.engine.mean_zero import MeanZero
...
class ConvLayer(TrainableLayer):
    ...
    def layer_op(self, input_tensor):
        ...
        if(self.k_constraint):
            output_tensor = tf.layers.conv3d(
                                          inputs = input_tensor,
                                          filters=self.n_output_chns,
                                          kernel_size=self.kernel_size,
                                          strides=full_stride,
                                          dilation_rate=full_dilation,
                                          padding=self.padding,
                                          kernel_constraint = MeanZero([0,1,2,3]),
                                          activation = 'relu',
                                          kernel_regularizer = self.regularizers['w'],
                                          kernel_initializer= self.initializers['w'],
                                          bias_initializer= self.initializers['b'],
                                          bias_regularizer = self.regularizers['b'],
                                          name='conv'
                                          )
        else:
